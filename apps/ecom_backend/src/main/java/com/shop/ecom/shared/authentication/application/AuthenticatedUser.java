package com.shop.ecom.shared.authentication.application;
...
public final class AuthenticatedUser {
  ...
  public static String readPrincipal(Authentication authentication){
    ...
    if(authentication instanceof JwtAuthenticationToken token){
      return (String) token.getToken().getClaims().get(PREFERED_USERNAME);
    }
    if(authentication.getPrincipal() instanceof DefaultOAuth2User oidcUser){
  ...
  private static Function<Authentication, Roles> toRoles(){
    return authentication -> new Roles(authentication
      .getAuthorities().stream().map(GrantedAuthority::getAuthority).map(Role::from)
